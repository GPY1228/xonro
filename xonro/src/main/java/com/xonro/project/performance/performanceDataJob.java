...
public class performanceDataJob implements IJob {
    /**
     * 每月一号创建绩效指标数据
...
//        month = 6;
        ...
        for (RowMap rowMap : orgUsers) {
            ...
            if (userId.equals( "Lion.Li" )||userId.equals( "Henry.Chen" )){
                continue;
            }
            ...
            if (engineers.contains( roleId )) {
                //流程ID
                ProcessInstance processInstance=SDK.getProcessAPI().createProcessInstance( "obj_55d5ca1ad771465f85bbb1ef4a7b99ff","admin","绩效台账创建" );
                //创建数据
                //主表数据
                BO bo = new BO();
                ...
                bo.set( "SCORE", 100 );
                bo.set( "ROLE_NAME" ,"1");
                SDK.getBOAPI().create( "BO_XR_CHECK_DATA", bo, processInstance, UserContext.fromUID( userId ) );
...
//            rulerBiz1.DayRuler( userId, "6" );
//            try {
//                rulerBiz1.DeductRuler( userId, "6" );
...
//            }
            SDK.getBOAPI().create( "BO_XR_CHECK_LIST", gridBo, processInstance, UserContext.fromUID( "admin" ) );
        }
        return list;
    }
}
